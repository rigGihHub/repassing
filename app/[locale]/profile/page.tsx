...
export default async function ProfilePage({params}: {params: Promise<{locale: string}>}) {
  ...
  return <main className="accountShell">
    ...
    <div className="profileActions"><Link className="primary inlineAction" href={`/${locale}/sell`}>＋ {sv?'Skapa annons':'Create listing'}</Link></div>
    ...
    {!session.preview && <section className="accountPanel sellerListings"><h2>{sv?'Mina annonser':'My listings'}</h2>{sellerListings.length===0?<p>{sv?'Du har inga annonser ännu.':'You have no listings yet.'}</p>:<div className="membershipList">{sellerListings.map((l:any)=><div className="membershipRow" key={l.id}><div><strong>{l.title}</strong><span>{l.status}</span></div><b>{new Intl.NumberFormat(sv?'sv-SE':'en-GB',{style:'currency',currency:l.currency,maximumFractionDigits:0}).format(Number(l.price_minor)/100)}</b></div>)}</div>}</section>}
